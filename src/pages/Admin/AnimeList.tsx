...
import { Plus, Edit, Trash2, Eye, EyeOff, Search, ArrowUpDown, Filter } from 'lucide-react';
...
export const AnimeList = () => {
  ...
  const filteredAndSorted = useMemo(() => {
    ...
    // Tab filter
    if (activeTab === 'published') result = result.filter(a => a.published);
    if (activeTab === 'drafts') result = result.filter(a => !a.published);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-yoru-surface border border-yoru-border rounded-xl overflow-hidden shadow-lg">
        <div className="p-4 border-b border-yoru-border flex flex-col md:flex-row md:items-center justify-between gap-4 bg-yoru-surface-elevated/30">
          
          {/* Tabs */}
          <div className="flex bg-yoru-bg/50 p-1 rounded-lg border border-yoru-border/50 self-start md:self-auto">
        ...
        <div className="overflow-x-auto min-h-[400px]">
          <table className="w-full text-left text-sm text-yoru-text">
            <thead className="bg-yoru-surface/80 text-xs uppercase tracking-widest text-yoru-text-muted border-b border-yoru-border">
              <tr>
                <th className="px-6 py-4 font-bold">Anime</th>
                <th className="px-6 py-4 font-bold hidden sm:table-cell">Status</th>
                <th className="px-6 py-4 font-bold hidden md:table-cell">Eps</th>
                <th className="px-6 py-4 font-bold">State</th>
                <th className="px-6 py-4 font-bold text-right">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-yoru-border/50">
              {isLoading ? (
                <tr>
                ...
                <tr key={anime.id} className="hover:bg-yoru-surface-elevated/30 transition-colors group">
                  ...
                  <td className="px-6 py-4 hidden md:table-cell text-zinc-400 font-mono text-xs">{anime.totalEpisodes || '-'}</td>
                  ...
                  <td className="px-4 sm:px-6 py-4 text-right whitespace-nowrap space-x-1 sm:space-x-2">
